...
add1=Add()([vgg.output,lstm.output])
add1_tanh=Activation("tanh")(add1)

add1_reshape=Reshape((14*14,1024))(add1_tanh)
# p1=Dense(1, activation="softmax")(add1_reshape)
p1=Dense(1)(add1_reshape)
p12=Reshape([-1])(p1)
p13=Activation("softmax")(p12)
# p1=Flatten()(p1)
# p1=Reshape((14,14,1))(p1)

# def mask_image(tensors):
#     return(tensors[0]*tensors[1])

add1_reshape_per=Permute((2,1))(add1_reshape)
wei_mul_1=Multiply()([add1_reshape_per,p13])
wei_sum_1=Lambda(lambda x:K.sum(x,axis=2,keepdims=True))(wei_mul_1)
ques_2_fl=Flatten()(wei_sum_1)
ques2=Add()([lstm.output,ques_2_fl])


# wei_mul_1=Lambda(mask_image)([vgg.output,p1])
# wei_sum_1=Lambda(lambda x:K.sum(x,axis=2,keepdims=True))(wei_mul_1)
# wei_sum_1=Lambda(lambda x:K.sum(x,axis=1,keepdims=True))(wei_sum_1)

# wei_sum_1=Reshape([-1])(wei_sum_1)
# ques2=Add()([lstm.output,wei_sum_1])

# img_ten_den2=Dense(1024)(vgg.output)
ques_feat_den2=Dense(1024)(ques2)
add2=Add()([vgg.output,ques_feat_den2])
add2_tanh=Activation("tanh")(add2)

add2_reshape2=Reshape((14*14,1024))(add2_tanh)
p2=Dense(1)(add2_reshape2)
p22=Reshape([-1])(p2)
p23=Activation("softmax")(p22)
# p2=Reshape((14,14,1))(p2)

add2_reshape_per2=Permute((2,1))(add2_reshape2)
wei_mul_2=Multiply()([add2_reshape_per2,p23])
wei_sum_2=Lambda(lambda x:K.sum(x,axis=2,keepdims=True))(wei_mul_2)


# wei_mul_2=Lambda(mask_image)([vgg.output,p2])
# wei_sum_2=Lambda(lambda x:K.sum(x,axis=2,keepdims=True))(wei_mul_2)
# wei_sum_2=Lambda(lambda x:K.sum(x,axis=1,keepdims=True))(wei_sum_2)

# wei_sum_2=Reshape([-1])(wei_sum_2)

ques_2_fl2=Flatten()(wei_sum_2)
ques3=Add()([ques2,ques_2_fl2])
den=Dense(1024)(ques3)
den_ac=LeakyReLU()(den)
pred=Dense(len(class_to_label),activation="softmax")(ques3)


# In[ ]:


model_vqa=Model(inputs=[vgg.input,lstm.input],outputs=pred)
...
callback = [EarlyStopping(monitor='val_loss',mode="min",verbose=1, patience=30),
             ModelCheckpoint('train_{}_{}.h5'.format("vgg","lstm_baseline_san"), monitor='val_loss',mode="min" ,verbose=1,save_best_only=True)]
...
model=load_model("train_vgg_lstm_baseline_san.h5")
